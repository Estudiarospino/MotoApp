...
const ESTADO_OPCIONES = [
  ...
  { value: "en_contrato", label: "En contrato" },
...
const CHIPS_MOVIL = [
  { key: "todos", label: "Todas", estado: "todos" },
  { key: "disponible", label: "Disponibles", estado: "disponible" },
  { key: "en_contrato", label: "En contrato", estado: "en_contrato" },
  { key: "vendida", label: "Vendidas", estado: "vendida" },
] as const;

const VALORES_DEFECTO: MotosFiltrosValor = { q: "", estado: "todos", marca: "todas", orden: "reciente" };

export function MotosFilters({
  basePath,
  valores,
  marcas,
}: {
  basePath: string;
  valores: MotosFiltrosValor;
  marcas: string[];
}) {
  ...
  function navegar(cambios: Partial<MotosFiltrosValor>) {
  ...
  const hayFiltrosActivos =
    valores.q !== "" || valores.estado !== "todos" || valores.marca !== "todas" || valores.orden !== "reciente";
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar motos..." className="pl-9" />
          </div>
          <button
            type="button"
        ...
        <div className="flex gap-2 overflow-x-auto pb-0.5">
          {CHIPS_MOVIL.map((chip) => {
            const activo = valores.estado === chip.estado;
            return (
              <button
                key={chip.key}
                type="button"
                onClick={() => navegar({ estado: chip.estado })}
                className={cn(
